...
## npm release and standalone verification

The release preparation adds `npm run test:package`. It installs the actual npm
archive into isolated temporary applications, checks declarations without
`skipLibCheck`, and runs browser lifecycle/reply/locale tests on React 18.0.0 and
19.3.0, Vue 3.3.0 and 3.5.43, and Angular 19.0.0 and 22.1.7. Plain JavaScript is
also checked without any optional framework peer installed. The copied Next.js
16.3.5 example builds and runs its two offline SSR/hydration/router tests using
the installed archive. Standalone tests load all three shipped HTML examples with
ordinary script tags; the backend example uses a local deterministic SSE fixture.

These checks caught incompatible Vue 3.3 declarations (fixed with stable public
types) and undeclared font/shared-file dependencies in the Next.js example (made
self-contained). The release archive's SHA-512 and results are recorded in
`.artifacts/package/verification.json`; `publish-verified.mjs` rejects a modified
archive. See [RELEASING.md](RELEASING.md) for the workflow and limits.
