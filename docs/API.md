...
Navigation allowlists accept exact pathnames and trailing `/*` patterns. For example, `allowedPaths: ['/', '/bounty/*']` allows `/`, `/bounty`, `/bounty/123` and `/bounty/123/history`, but not `/bounty-other/123`. Configure the same allowlist on the browser and server. The model supplies a concrete pathname such as `/bounty/123`; the wildcard itself is not a destination. Other glob patterns are not supported. External origins, path traversal and encoded path separators are rejected. The browser preserves query strings and fragments when passing an allowed destination to the host router; model navigation arguments contain only the pathname.
