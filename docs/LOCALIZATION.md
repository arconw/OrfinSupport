...
The selected `locale` travels with every chat request. The server validates the tag and explicitly instructs the model to answer in that language even when the question, conversation history, reference documents or tool results use another language. This policy remains in effect through tool calls. A visitor can explicitly request another response language; an English question alone does not change the chosen locale. Unknown language codes fall back to English; valid additional languages can use custom UI translations. This relies on the chosen model’s language capabilities. Existing messages remain unchanged; an in-flight answer retains the language selected when its request began.
