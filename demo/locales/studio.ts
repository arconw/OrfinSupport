...
const en: StudioCopy = {
  analysis:
    'Delivery rose from **48 to 66 tasks (+37.5%)** across equal three-week windows (Aug 10–30 and Aug 31–Sep 20). Brand grew **50% (24 → 36)**; Web grew **25% (24 → 30)**. Brand’s lead time fell 6 → 4 days and rework 6 → 3 tasks.\n\nThe review checklist introduced Aug 31 (**DL-041**) coincides with Brand’s improvement; this is an association, not proof of cause. Two documented client-feedback delays (**DL-052**) help explain Web’s slower lead time of 7 days. Last week: **24 of 26 planned tasks (92.3%)**. Source: Northstar delivery ledger v2026.09.20.',
  compare:
...
const copies: Record<string, StudioCopy> = {
  ...
  ru: {
    analysis:
      'За равные трёхнедельные периоды (10–30 августа и 31 августа–20 сентября) завершено **48 → 66 задач, рост 37,5%**. Brand: **24 → 36 (+50%)**, Web: **24 → 30 (+25%)**. У Brand срок выполнения сократился с 6 до 4 дней, задачи с доработкой — с 6 до 3.\n\nЧек-лист проверки появился 31 августа (**DL-041**). Он совпадает по времени с улучшением, но причинность не доказана. Две задержки обратной связи от клиента (**DL-052**) помогают объяснить более долгий срок Web — 7 дней. Последняя неделя: **24 из 26 задач, 92,3%**. Источник: Northstar delivery ledger v2026.09.20.',
    compare:
  ...
  fr: {
    analysis:
      'Sur deux périodes de trois semaines, les livraisons passent de **48 à 66 tâches (+37,5 %)**. Brand : **24 → 36 (+50 %)** ; Web : **24 → 30 (+25 %)**. Le délai de Brand baisse de 6 à 4 jours et les reprises de 6 à 3.\n\nLa checklist du 31 août (**DL-041**) coïncide avec cette amélioration, sans prouver une cause. Deux attentes de retour client (**DL-052**) éclairent le délai de Web, encore à 7 jours. Dernière semaine : **24 sur 26 tâches (92,3 %)**. Source : Northstar delivery ledger v2026.09.20.',
    compare:
...
const translated: Record<string, [string, string, string, string, string, string, string, string]> =
  {
    ...
    pt: [
      'Entregas: **48 → 66 tarefas (+37,5%)** em períodos iguais de três semanas. Brand: **24 → 36 (+50%)**; Web: **24 → 30 (+25%)**. Brand reduziu o prazo de 6 para 4 dias e retrabalho de 6 para 3. A lista de revisão (**DL-041**) coincide com a melhoria, sem provar causalidade. Duas esperas por clientes (**DL-052**) contextualizam os 7 dias de Web. Última semana: **24/26 (92,3%)**. Fonte: Northstar delivery ledger v2026.09.20.',
    ...
    ar: [
      'خلال فترتين متساويتين من ثلاثة أسابيع ارتفع التسليم **48 → 66 مهمة (+37.5%)**. Brand: **24 → 36 (+50%)**؛ Web: **24 → 30 (+25%)**. انخفض وقت Brand من 6 إلى 4 أيام وإعادة العمل من 6 إلى 3. تتزامن قائمة الفحص (**DL-041**) مع التحسن دون إثبات السببية. يفسر تأخران لرد العميل (**DL-052**) جزءًا من مدة Web البالغة 7 أيام. الأسبوع الأخير: **24/26 (92.3%)**. المصدر: Northstar delivery ledger v2026.09.20.',
    ...
    zh: [
      '在两个相等的三周周期中，完成任务从 **48 增至 66（+37.5%）**。Brand：**24 → 36（+50%）**；Web：**24 → 30（+25%）**。Brand 的交付周期从 6 天降至 4 天，返工任务从 6 降至 3。检查清单（**DL-041**）与改善同时出现，但不能证明因果关系。两次客户反馈等待（**DL-052**）部分解释了 Web 仍需 7 天。上周：**24/26（92.3%）**。来源：Northstar delivery ledger v2026.09.20。',
    ...
    ja: [
      '同じ3週間の期間で、完了数は **48 → 66件（+37.5%）**。Brand：**24 → 36（+50%）**、Web：**24 → 30（+25%）**。Brand の所要日数は6→4日、手直しは6→3件です。チェックリスト（**DL-041**）は改善と同時期ですが、因果関係は証明されていません。顧客回答待ち2件（**DL-052**）は Web の7日という期間を一部説明します。先週は **24/26件（92.3%）**。出典：Northstar delivery ledger v2026.09.20。',
    ...
    ko: [
      '동일한 3주 기간의 완료 작업은 **48 → 66건(+37.5%)**입니다. Brand: **24 → 36(+50%)**, Web: **24 → 30(+25%)**. Brand 소요 시간은 6→4일, 재작업은 6→3건입니다. 체크리스트(**DL-041**) 도입과 개선이 동시에 나타났지만 인과관계는 입증되지 않았습니다. 고객 응답 대기 두 건(**DL-052**)은 Web의 7일 소요 시간을 일부 설명합니다. 지난주 **24/26건(92.3%)**. 출처: Northstar delivery ledger v2026.09.20.',
