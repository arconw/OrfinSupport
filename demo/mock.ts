  ...
  return {
    async *stream(request, signal) {
      ...
      if (/tour|show me around|экскурс|покажи проект/.test(query) && request.features.tour) {
        ...
        /capacity|availability|загруз|свобод|capacidad|capacité|kapazität|capacidade|capacità|capaciteit|obciąż|завантаж|kapasite|قدرة|क्षमता|产能|稼働|가동/.test(
          query,
        ) &&
        request.features.tools
      ) {
        intent = 'capacity';
        events.push({
          type: 'tool',
        ...
        /plan|price|cost|billing|план|стои|цен|precio|prix|preis|preço|prezzo|prijs|cena|ціна|fiyat|سعر|कीमत|价格|料金|요금/.test(
          query,
        )
      ) {
        intent = 'plan';
      ...
      if (localized) {
        ...
        const sectionId =
          action?.type === 'action' && 'sectionId' in action.action
        ...
        else if (intent === 'privacy') reply = localized.privacy;
        else if (intent === 'tour') reply = `${ui.tour}. ${localized.followup}`;
        ...
        for (const event of events)
          if (event.type === 'sources')
            event.sources = event.sources.map((source) => ({
              ...source,
              title: source.id === 'plan' ? localized.planTitle : ui.sources,
              content: source.id === 'plan' ? localized.plan : localized.privacy,
            }));
      }
      for (const event of events) {
